...
The snippet is injected into Streamlit's static `index.html` at Docker
build time by `scripts/patch_streamlit_index.py`. `st.html` strips
`<script>` tags and `st.components.v1.html` runs inside an iframe, so
neither can expose `window.posthog` on the parent page.
"""
